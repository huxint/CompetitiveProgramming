...
BigInteger BigInteger::pow(const long long &x, const BigInteger &p) const {
    ...
    for (long long t = x; t != 0; a = a * a % p, t >>= 1) {
        if (t & 1) {
            res = res * a % p;
        }
    }
...
BigInteger BigInteger::root(const long long &m) const {
    ...
    BigInteger n = *this, t = base,
               x0 = std::min(n, t._move_l((n.size + m) / m));
...
BigInteger BigInteger::operator&(const BigInteger &x) {
    return binary_op_helper(*this, x, [](bool a, bool b) -> bool {
        return a & b;
    });
}
BigInteger BigInteger::operator|(const BigInteger &x) {
